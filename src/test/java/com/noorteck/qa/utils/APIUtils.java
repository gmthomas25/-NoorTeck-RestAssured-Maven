...
//		System.out.println(" query map "+queryParamMap);
	...
	public static void setRequestBodyTestData(String regionName, String testDataFileName, Integer index)throws JSONException, IOException {
	...
	public static String getRequestBody(String requestBodyFileTempaltePath, Map<String, Object> testDataMap)
		...
		String query = "select manager_id, department_id, job_id FROM hr_scrum.employees WHERE manager_id IS NOT NULL ORDER BY RAND() LIMIT 1;";
		...
		while (list.hasNext()) {
			...
			switch(key) {
				case "jobId":
					value = DBUtils.makeDBRequest(query).get(0).get("job_id");
					obj.put(key, value);
					break;
				case "managerId":
					value = DBUtils.makeDBRequest(query).get(0).get("manager_id");
					obj.put(key, value);
					break;
				case "departmentId":
					value = DBUtils.makeDBRequest(query).get(0).get("department_id");
					obj.put(key, value);
					break;
				default:
		...
		while (keys.hasNext()) {
			...
			regionDataMap.put(key, obj.get(key));
		}
		
		
		modifyTestData(regionDataMap, "", "",0);
...
//		}
	...
	public static void modifyTestData(Map<String, Object> map, String fileName, String region, int index)
		...
		String filePath = "src" + File.separator + "test" + File.separator + "resources" + File.separator + "." + File.separator + "NoorTeck-RestAssured-Maven" + File.separator + "src" + File.separator + "test" + File.separator + "resources" + File.separator + "data" + File.separator + "emp-api" + File.separator + "EmpTestData.json";
		Paths.get("src", "test", "resources", ".", "NoorTeck-RestAssured-Maven", "src", "test", "resources", "data", "emp-api", "EmpTestData.json").toString();

		String jsonToString = JsonFileUtils.jsonToStrConvertion(filePath);
		JSONObject obj = new JSONObject(jsonToString);
		JSONArray regionArr = obj.getJSONArray("scrum");
		JSONObject regionData = regionArr.getJSONObject(0);
		...
		FileWriter file = new FileWriter("./src/test/java/com/noorteck/qa/data/emp-api/EmpTestData.json", false);
